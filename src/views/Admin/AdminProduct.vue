    ...
    <div class="container-fuild">
      ...
      <div class="row m-0 p-0" >
        ...
        <div class="col-md-10 p-2" >
          ...
          <router-link :to="{ name: 'addproduct' }" class="nav-link text-white">
          ...
          <table class="table table-hover">
            ...
            <tr v-for="(product, index) in products" :key="product._id">
              ...
              <td><img id="hinhsp" :src="'http://localhost:3000/'+ product.image"></td>
              <td>{{product.noibat}}</td>
              <td>{{product.moi}}</td>
              <td>{{Intl.NumberFormat('vi-VN', {
                    style: 'currency',
                    currency: 'VND',
                  }).format(product.giaban)}}</td>
              <td>{{Intl.NumberFormat('vi-VN', {
                    style: 'currency',
                    currency: 'VND',
                  }).format(product.giakm)}}</td>
              <td v-if="product.dangban == 1">Đang bán</td><td v-else>Tạm dừng</td>
              <td>
                <div>
                  <button @click="goNhapKho(product._id)" type="submit" class="btn btn-primary">Nhập kho</button>
                  <button @click="goEditProduct(product._id)" type="submit" class="btn btn-secondary">Cập nhật</button>
                  <button @click="deleteProduct(product._id)" type="submit" class="btn btn-danger">Xóa</button>
                </div>
              </td>
